...
class MovenetRenderer:
    def __init__(self, pose, output=None):
        self.pose = pose
        ...
        if output is None:
            ...
            self.output = cv2.VideoWriter(
                output, fourcc, pose.video_fps, (pose.img_w, pose.img_h)
            )

    def draw(self, frame, body):
        self.frame = frame
        body.keypoints = np.append(
            body.keypoints, [(body.keypoints[6] + body.keypoints[5]) / 2], axis=0
        ).astype(int)
        body.scores = np.append(
            body.scores, [(body.scores[5] + body.scores[6]) / 2], axis=0
        )
        lines = [
            np.array([body.keypoints[point] for point in line])
            for line in LINES_BODY
            if body.scores[line[0]] > self.pose.score_thresh
            and body.scores[line[1]] > self.pose.score_thresh
        ]
        ...
        if self.show_crop:
            cv2.rectangle(
                frame,
                (body.crop_region.xmin, body.crop_region.ymin),
                (body.crop_region.xmax, body.crop_region.ymax),
                (0, 255, 255),
                2,
            )
