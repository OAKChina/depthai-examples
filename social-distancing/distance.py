...
class DistanceGuardian:
    ...
    def parse_frame(self, frame, detections):
        ...
        results = sorted(results,key=lambda x: x.get('distance'),reverse=True)
...
class DistanceGuardianDebug(DistanceGuardian):
    def parse_frame(self, frame, boxes):
        ...
        for result in results:
            ...
            cv2.putText(overlay, str(round(result['distance'], 1)), (label_x, label_y), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255,0,0), 1)
