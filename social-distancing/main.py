...
class Main:
    #实例化DepthAI，DistanceGuardian，AlertingGate三个类
    ...
    def parse_frame(self, frame, results):
        #调用DistanceGuardian类的parse_frame函数,将图片信息和对象描述数组传入
    ...
    def parse_frame(self, frame, results):
        ...
        for result in distance_results:
            if result['dangerous']:
                ...
                too_close_ids.append(result['detection1'])
                left, right = self.calc_x(result['detection2'].depth_x)
                top, bottom = self.calc_z(result['detection2'].depth_z)
                cv2.rectangle(bird_frame, (left, top), (right, bottom), (0, 0, 255), 2)
                too_close_ids.append(result['detection2'])

        for result in results:
            if result not in too_close_ids:
        ...
        cv2.moveWindow("Bird", 500, 200)
        cv2.moveWindow("Frame", 600, 200)
        key = cv2.waitKey(1)
